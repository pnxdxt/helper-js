...
const getHandler = key => {
  switch (key) {
    case 'class':
    case 'className':
      return mergeCssClass;
    case 'dataset':
    case 'style':
      return deepAssignAttr;
    default: {
...
const createElement = (args, props, child) => {
  ...
  if (props || args.props) {
    const mergeProps = each(
      (value, key) => isDef(value) && getHandler(key)(elem, value, key)
    );
...
const prepareStyleArgs = (tag, style) =>
  isStr(tag)
    ? prepareArgs(tag, {style: style.style || style})
    : prepareArgs('div', {style: tag.style || tag});

const extend = (args, props) => preparedH(mergePropsDefault(args, props));
...
h.replaceContent = curry((el, content) => {
  empty(el), appendChild(el, content);
});
